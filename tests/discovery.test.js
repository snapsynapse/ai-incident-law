...
test("published release identity and install identity remain explicit", () => {
  ...
  assert.ok(Number(publication.release_source.version.split(".")[2]) <= Number(pkg.version.split(".")[2]));
  assert.equal(publication.release_source.status, "published-release");
  assert.equal(discovery.hosted_source.version, publication.release_source.version);
  ...
  assert.match(readme, /"args": \["-y", "ai-incident-law@VERSION"\]/);
  assert.match(readme, /Replace: VERSION -> an npm-verified version/);
  assert.match(readme, /Customize\n```json/);
  assert.match(readme, /Do not infer a package's publication status from this source README\./);
