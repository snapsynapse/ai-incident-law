...
async function collect({matters,streams,index=[],state=core.emptyState({reviewPolicy:{owner:'Sam Rogers',capacityMinutesPerWeek:null,scope:'six-repo-portfolio'}}),now=new Date().toISOString(),fetcher=fetchPublication,parseIndex,retainRaw=async()=>{},persist=async()=>{},maxRequests=7}={}){
 ...
 for(const source of sources){
  ...
  if(source.kind==='matter'&&qualified){
   ...
   if(!unchanged){const evidenceHash=kind==='baseline_source_review'?contentHash:hash({contentHash,previousHash:prior?.normalizedContentHash ?? null});const {finding}=core.upsertFinding(state,{subjectIds:[source.id],claim:'Review active matter source snapshot',oldValue:{filingStatus:source.baselineStatus,attribution:source.attribution},newValue:{sourceRole:source.role,sourceHash:contentHash,qualification:source.qualification,assessment:kind},evidence:{sourceId,normalizedContentHash:evidenceHash,locator:source.url},affectedRecords:['data/data.json'],reviewDueAt:new Date(Date.parse(now)+7*86400000).toISOString()},{now});summary.findingId=finding.id;}
